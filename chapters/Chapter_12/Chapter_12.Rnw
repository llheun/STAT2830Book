...
\item Put into words the concepts of total, unplained, and explained variation.
...
Based on the above graphic (Figure: 2.1), there appears to be a relationship between the two variables -- height and self esteem.  But is this relationship real?  We will use the five step model to confirm our initial assumption. 
...
\item Step 4.  Computing the Test Statistic.
  ...
  \begin{figure}[ht]
    \centering
    \includegraphics[width=6.5cm]{chapters/Chapter_12/ext_figure/zCorrA.png} % requires the graphicx package
...
Since $4.41 > 2.01$, we reject the null hypothesis.  Thus we can say that strong, positive relationship exists between male height and male self estemm.
