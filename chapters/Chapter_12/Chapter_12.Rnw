...
The definition of assocation states that the relationship of two variables will vary in the same direction either positive or negative.  As the \emph{X} variable increases, the \emph{Y} variable will increase (decrease if the direction is negative).  In other words, the \emph{X-Y} pair represents a point on the scatter plot.  If there is a relationship between these two variables
