...
National Opinion Research Center (NORC) (U of C) administers the General Social Survey (GSS), and is a primary source of data on social attitudes in the U.S. \cite{GSS2014}  Once each year; NORC employees interview 1500 adults in their homes all across the country.  The subjects are asked their opinions on sex and marriage, attitudes toward women, welfare, foreign policy, and many other issues.
...
\subsubsection{Interpretation}
 The percent  of persons who indicate that voting is a crucial responsibility of a citizen is 62.41 percent.  When we consider the differences between the frequencies of an event to the percentages of that event, we quickly see the percentages are standardized and thus are easily understood because they vary from zero to 100.  Therefore, they are very useful when comparing populations of different sizes when we look at Table 2.2.  Even though the totals of females and males are different, we see that the percentages for all categories are not very different.
...
A {\bf{ratio}} is a relation between two numbers point to the number of times the first number contains the second. For example, a local hospital reported annual birth ratio at their facility as 102 males and 98 females (that is 102:98, which is equivalent to the ratio 51:49).  Consequently, it can also be a fraction such as 1.04.  We can also change the positions of the numbers.  Consider our current example, the hospital annual birth ratio is 98 females and 102 males or 98:102 (or 49:51).
...
\item Histograms have areas that are proportional to the frequency; i.e., in Table 2.7, we see that the rate for the sub-group 10-12 years is \fbox{823} of 2538 or 32.4 percent.  In this text, we will set the bars to the same width. 
