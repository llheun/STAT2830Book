...
A {\bf{ratio}} is a relation between two numbers that indicate the number of times the first number contains the second. For example, a local hospital reported annual birth ratio at their facility as 102 males and 98 females (that is 102:98, which is equivalent to the ratio 51:49).  It can also be a fraction such as 1.04 males to 1 female.  We usually multiply both numbers by 100 to have no decimals.  In other words, 104:100.  We can also change the positions of the numbers.  Consider our current example; the hospital annual birth ratio is 98 females and 102 males or 98:102 (or 49:51).

The comparison of two numbers in a ratio can be any quantities of a comparable kind, such as dimensions, objects, persons,  or volumes.  A ratio is written "x to y" or x:y, or sometimes expressed arithmetically as a quotient of the two.[2] When the two quantities have the same units, as is often the case, their ratio is a dimensionless number.  If the variables have different units, the quotient is a rate such as miles per qallan.  But in many applications, the word ratio is often used instead for this more general notion as well.[3]
...
\end{eqnarray}
  ...
  Notice that we use 12 as $f_1$ since this the earlier time (2012 versus 2014). The percent change results in a 34.2 percent decrease in the homicide rate. The percent change is consistent with the reported rates since 7.9 is a reduction over 12.
