...
After reviewing this data from the GSS (See Table: 2.1), compute the  percent of persons who indicate that voting is a very important responsibility of being a citizen.
...
& Count & Percent \\ 
 \hline
 Not at All Important & 53 & 4.28 \\ 
 2 & 21 & 1.70 \\ 
 3 & 40 & 3.23 \\ 
 4 & 77 & 6.22 \\ 
 5 & 119 & 9.62 \\ 
 6 & 155 & 12.53 \\ 
 Very Important  & \fbox{772} & 62.41 \\ 
  \hline
...
\subsubsection{Interpretation}
 The percent  of persons who indicate that voting is a very important responsibility of a citizen is 62.41 percent.
...
%\begin{itemize}
%\item 
where \emph{f} is the number of cases in a category and \emph{n} is the number of cases in all categories.
%\end{itemize}
...
After reviewing this data from the GSS (See Table: 2.2), compute the percent of females who indicate that voting is a very important responsibility of being a citizen.
...
&  \multicolumn{2}{c}{Gender}  \\ \hline
 & Male & Female \\ \hline
Not at All Important &24&29\\
2 & \fbox{7} & \fbox{14} \\
3 &20& 20\\
4 &36& 41\\
5 &50& 69\\
6 &84& 71\\
Very Important & 320& \fbox{452} \\ \hline
Total & 541      & \fbox{696} \\ \hline
\end{tabular}
\end{table}
...
\subsubsection{Interpretation}
 The percent  of women who indicate that voting is a very important responsibility of a citizen is 64.94 percent.
...
After reviewing the data from the GSS (See Table 2.2), compute the ratio  of females to males who indicate that voting  is not very important responsibility (level 2) of being a citizen.  The GSS survey indicated 14 females and 7 males, compute the following: 

\begin{enumerate}
\item  The ratio of females to males: 

\begin{equation}
Ratio = \frac{f_1}{f_2} = \frac{f_{females}}{f_{males}} = \frac{14}{7} = \frac{2}{1} 
\end{equation}

{\bf{Interpretation}}  \\
This means that there are two females for every   male,  who indicate that voting  is not very important responsibility (level 2) of being a citizen.


\item   The ratio of males to females:

\begin{equation}
Ratio = \frac{f_1}{f_2} = \frac{f_{males}}{f_{females}} = \frac{7}{14} = \frac{1}{2}
\end{equation}

{\bf{Interpretation}}  \\
This means that there is a male for every two  females,  who indicate that voting  is not very important responsibility (level 2) of being a citizen.
...
Suppose Kalamazoo  (population of 75,000) had 9 homicides in 2012 [3], what is the homicide rate per (100,000) in this town.
...
\end{equation}
  % \begin{itemize}
  % \item 
  which is read as 12 homicides per 100,000 population.  This is a convenient comparison statistic between groups of widely differing populations.
  % \end{itemize}
...
\item Rates are usually multiplied by some power of 10 to eliminate leading zeros.
...
Suppose Kalamazoo had a homicide rate of 12 in 2012 and in 2014, it had a homicide rate of 7.9 (reported as homicides per 100,000 population).
...
\end{eqnarray}
  ...
  % \begin{itemize}
  % \item 
  Notice that 12 is used as $f_1$ since this the the earlier time (2012 versus 2014).  This results in a 34.2 percent decrease in the homicide rate.  This is consistent with the reported rates since 7.9 is a decrease over 12.
  % \end{itemize}
...
\end{equation}
  % \begin{itemize}
  % \item 
  where $f_1$ is the first frequency (starting point) and $f_2$ is the second frequency (ending point).  The results express the size (and direction) of the change in scores $(f_2 - f_1)$ as a percentage to the score at the earlier time $(f_1)$.
  %\end{itemize}
...
Using the percent column from Table 2.6, let's use a bar chart to summarize  the marital status from the GSS 2014 survey.  The bars represent the categories and the heights of the bars correspond to the percent or number of cases in each category.  The bar chart always has gaps between the bars to indicate categorical type of data.

<<msbar, echo=FALSE, fig.pos="ht", fig.align="center", fig.width=4.7, fig.height=4.7, fig.cap="Marital Status Barchart from GSS 2014 Survey">>=
...
Using the percent column from Table 2.6, let's use a pie chart to summarize  the marital status from the GSS 2014 survey.  The slices represent the categories and the size of the slice corresponds to the percent of cases in each category.

<<mspie, echo=FALSE, fig.pos="ht", fig.align="center", fig.width=4.7, fig.height=4.7, fig.cap="Marital Status Pie Chart from GSS 2014 Survey">>=
...
Numerical data is the second type of random variable (See Figure 2.4) that we can summarize. As we explained earlier in this chapter,  categorical variables can only be counted where as numerical variables  can be summarized using the following procedures:
...
Earlier in this chapter we looked at the GSS 2014 is a sociological survey used to collect data on demographic characteristics and attitudes of residents of the United States. The survey is conducted face-to-face with an in-person interview by the National Opinion Research Center at the University of Chicago, of adults (18+) in randomly selected households.  An example of a survey question is about the years of education of the household.  The data, from this survey, show the maximum years of education of the household.  A frequency distribution  provides an accurate picture of the overall years of education structure.  But before we can build a frequency distribution table, we categorized the numbers by using the following steps:  

\begin{enumerate}
\item Determine number of classes, minimum, maximum, range and interval size.
\item \# of classes: generally, we take the square root of the sample size, $\sqrt{2538} = 50.4$, but in this case, this value exceeds 10 which is the maximum number of suggested bars.
\item $minimum = 0$
\item $maximum = 20$
\item $range = 20 - 0 $
...
<<mshist, echo=FALSE, fig.pos="ht", fig.align="center",  fig.width=4.7, fig.height=4.7>>=
...
\begin{itemize}
\item The bars of a histogram represent the subcategories and the heights of each bar corresponds to the percentage or number of cases in each subcategory.

\item Histograms have areas that are proportional to the frequency; i.e., in Table 2.7, we see that the frequency for the sub-group 10-12 years is \fbox{823} of 2538 or 32.4 percent.  In this text, we will set the bars to the same width. 
...
\item Histograms are very useful in examming the ``shape'' of the structure (distribution), for example: Bell-shaped (symmetrical),  Skewed (left or right),   Multi-modal.
...
The objective of this chapter was to use descriptive statistics to summarize categorical and numerical data.  We built frequency tables and charts and graphs to tell others about our research.  We also computed percentages, proportions, ratios, and percent changes all to communicate our results to an outside audience.  We were able to use these descriptive statistic to build graphs (boxplots, charts (bar, line, and pie), and histograms).  These graphical presentations often help our audience to comprehend the complexity of the problem that we are examining.
...
\begin{exercises}    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  \begin{exercise}  % 1
    ...
    250 individuals were randomly selected and 195 of them do not smoke. What proportion of individuals are smokers?
	  \begin{enumerate}
	  \samepage
	  \item 78
	  \samepage
  ...
  \begin{exercise} % 2
    ...
    250 individuals were randomly selected and 195 of them do not smoke.  What is the ratio of nonsmokers to smokers?
	  \begin{enumerate}
	  \item 0.78
	  \item 0.28
	  \item 3.55
  ...
  \begin{exercise} % 3
    ...
    In a city of a population 25,000, there were 13 murders reported in the prior year.  What is the rate of murder in this city per 100,000?
	  \begin{enumerate}
	  \samepage
	  \item 5.2
	  \samepage
...
%      \begin{figure}[htbp]
%        \centering
        \includegraphics[width=7.9cm]{chapters/Chapter_2/ext_figure/spss2-3.png} % requires the graphicx package
...
%      \begin{figure}[htbp]
%        \centering
        \includegraphics[width=6.9cm]{chapters/Chapter_2/ext_figure/spss2-1.png} % requires the graphicx package
