...
\begin{itemize}
\item The bars represent the subcategories and the heights of each bar corresponds to the percentage or number of cases in each subcategory.
...
\item Histograms are very useful in examming the ``shape'' of the distribution, for example: Bell-shaped and symmetrical.  Skewed (left or right),   Multi-modal.
