...
After reading chapter two and doing the exercises, you will be able to:
\begin{itemize}
...
National Opinion Research Center (NORC) (U of C) administers the General Social Survey (GSS), and is a primary source of data on social attitudes in the U.S. \citep{GSS2014}  Once each year; NORC employees interview 1500 adults in their homes all across the country.  The subjects are asked their opinions on sex and marriage, attitudes toward women, welfare, foreign policy, and many other issues.
...
\subsubsection{Interpretation}
 The percent  of persons who indicate that voting is a crucial responsibility of a citizen is 62.41 percent.
...
\item To convert a percent to fraction, divide it by 100.
\end{itemize}
...
1 Not at All Important &24&29\\
2 & \fbox{7} & \fbox{14} \\
3 &20& 20\\
4 &36& 41\\
5 &50& 69\\
6 &84& 71\\
7 Very Important & 320& \fbox{452} \\ \hline
...
The \emph{ratio} is a relation between two similar magnitudes on the number of times the first contains the second.

\subsubsection{GSS Example continued}

After reviewing the data from the GSS (See Table 2.2), compute the ratio  of females to males who indicate that voting is not vital responsibility (level 2) of being a citizen.  The GSS survey showed that 14 females and 7 males, compute the following: 
...
{\bf{Interpretation}}  \\
The ratio indicates that there are two females for every male,  which denotes that voting  is not very important responsibility (level 2) of being a citizen.
...
{\bf{Interpretation}}  \\
The ratio indicates that there is a male for every two females,  which denotes that voting  is not very important responsibility (level 2) of being a citizen.
...
Suppose Kalamazoo  (population of 75,000) had nine homicides in 2012 \citep{Homicide2016}, what is the homicide rate per (100,000) in this town.
...
Suppose Kalamazoo had a homicide rate of 12 in 2012 and 2014, it had a homicide rate of 7.9 (reported as homicides per 100,000 population).
...
\end{eqnarray}
  ...
  Notice we use 12 as $f_1$ since this the earlier time (2012 versus 2014). The percent change results in a 34.2 percent decrease in the homicide rate. The percent change is consistent with the reported rates since 7.9 is a reduction over 12.
 
  % \end{itemize}
  

\item The percent change measures social change.  It is telling us
 how a variable has increased (positive rate) or decreased (negative rate) over a determined span of time.
...
Using the percent column from Table 2.6, let's use a bar chart to summarize  the marital status from the GSS 2014 survey.  The bars represent the categories, and the heights of the bars correspond to the percent or number of cases in each category.  The bar chart always has gaps between the bars to indicate a specific type of data.
...
Numerical data is the second type of random variable (See Figure 2.4) that we can summarize. As we explained earlier in this chapter,  categorical variables can only be counted whereas numerical variables  can be summarized using the following procedures:
