...
A {\bf{ratio}} is a relation between two numbers that indicate the number of times the first number contains the second. For example, a local hospital reported annual birth ratio at their facility as 102 males and 98 females (that is 102:98, which is equivalent to the ratio 51:49).  In other words, it is the quantitative (numeric) relation between two amounts showing the number of times one value contains or is contained within the other.  \cite{ratio2016}  It can also be a fraction such as 1.04 males to 1 female.  We usually multiply both numbers by 100 to have no decimals.  In other words, 104:100.  We can also change the positions of the numbers.  Consider our current example; the hospital annual birth ratio is 98 females and 102 males or 98:102 (or 49:51).
