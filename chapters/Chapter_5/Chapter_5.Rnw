...
The \emph{normal curve} is the single most important theory in the social sciences.  It has properties that are found in our world, and the distribution of the collected data follow the bell-shaped curve.  The normal distribution has special attributes that make it possible to tell in advance the proportion of a population where the shape of the data values have a bell-shaped distribution.  Later, we will discuss tests for differences between populations where the data follows the normal curve.  Here are some examples of the normally distributed data.
...
We will use the following to illustrate the empirical rule.
...
First we need to find the two associated z-scores for these two values (110 an 120):
...
\begin{itemize}
\item For the AUC to the right of the mean (between $0.00$ and $0.50$): we can get by using the symmetry of the normal curve and look up $0.50$ for column (b) (area between mean and Z score) in the table.  When we look in the table the associated area is 0.1915.
