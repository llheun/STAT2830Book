...
After reading chapter five and doing the exercises, you will be able to
\begin{itemize}
...
table to find areas above, below and between points of the curve and  express them regarding of probabilities.
...
\item Unimodal.
\item The mean divides the bell-shaped curve into two halves.
\item mean = median = mode

\item The mean and standard deviation determines the Normal Distribution curve.
...
$\bar{x} \pm 2SD$
\samepage 
\item 99.7\% of the observations are within three standard deviations (SD) of the mean or a total of 6 SD \\
$\bar{x} \pm 3SD$
\end{itemize}
...
We will use the following regarding the empirical rule.

Suppose you are concerned about the amount of money that students pay for their lunch.  The average meal costs five (5) dollars with a standard deviation of \$0.50.  Use the empirical rule to determine the three 68-95-99.7 percent of student lunch expense. 
...
The distribution of the IQ scores is normal or is bell-shaped.  Let's consider the following simulated example (i.e., ``faked data'') which is a good illustration of the bell-shaped curve and z-score.
...
assumption that we can use the normal distribution.  This fact allows us to analyze the results using the normal curve.
...
\item Making the following assumptions about a data set (distribution) will ultimately determine the particular distribution that we need to work.
	...
	\end{itemize}
\item The further statistical analysis will be based on the normal curve chosen.
