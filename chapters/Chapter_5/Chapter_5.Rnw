...
After reading chapter five and doing the exercises, we will be able to
\begin{itemize}
...
Given a bell-shaped distribution of measurements, approximately 
\begin{itemize}
\samepage 
...
Suppose we are concerned about the amount of money that students pay for their lunch.  The average meal costs five (5) dollars with a standard deviation of \$0.50.  Use the empirical rule to determine the three 68-95-99.7 percent of student lunch expense. 
...
\item We can use the symmetry of the distribution to get the result.

\item For example, if we want the proportion {\bf{below}} a z-score of $-1.5$ (or below an IQ score of 70 for males), it is just equal to the proportion {\bf{above}} a  z-score of 1.5.
