...
We are asked to find the proportion of men that have an IQ score higher than 130.
...
\begin{itemize}
\item We interpret 0.0668 (or 6.68\%) as an area under the curve representing all male individuals have an IQ score above 130 or the associated Z-score above 1.50.
...
\begin{itemize}
\item For the AUC to the left of the mean (between $0.00$ and $0.50$): we can get by using the symmetry of the normal curve and look up $0.50$ for column (b) (area between mean and Z score) in the table.  When we look in the table the associated area is 0.1915.
...
First we need to locate the area of the probability of 0.900 and the associated Z-score for this likeihood.
...
\newpage
  ...
  \begin{exercise} % 4 
    ...
    Given the distribution of male weights with a mean of 155 and standard deviation of 12, what value 
represents the $90^{th}$ percentile for weights of men?
	  \begin{enumerate}
	   \item 155.00 pounds
	  \item 170.36 pounds
	  \item 179.00 pounds 
  ...
  \begin{exercise} % 5 
    ...
    Given the distribution of male weights with a mean of 155 and standard deviation of 12, what value 
represents the $25^{th}$ percentile for weights of men?
	  \begin{enumerate}
	   \item 155.00 pounds
	  \item 163.10 pounds
	  \item 146.91 pounds 
...
\end{table}
  ...
  The mean and standard deviaion are 8.30 and 3.224, respectively.  What countries are in the top 15 percent?  Hint: find the value of the $85^{th}$ percentile and compare it to the Percentage of GDP.
...
@
  ...
  tmp51 <- qnorm(.15, mean=8.3, sd=3.224)

@

	  The value of the $15^{th}$ percentile is \Sexpr{tmp51}.
