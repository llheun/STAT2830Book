...
The following table gives the number of active social workers per 100,000 population in each state.  
...
\item Whenever we look at a new dataset we need to consider the following issues:

\item Researchers have a choice of how to analyze the data.  They can simply count the number of social workers per state, or we can transform the number of social workers per state to the number of social workers per 100,000 people per state.  Since the populations of each state vary, the number of social workers per 100,000 people provides a better measure of how many social workers are available to serve each state.

\item The next step, in data analysis is check the distribution of the data.  We can use either a histogram or a box plot.  Let's review Figure 3.2.  We find that the data is right skewed.

\item Now we need to determine the best measure of  location or central tendency.  Since this dataset is skewed, the best measure of location is the median (\Sexpr{md1})  since the mean (\Sexpr{mn1}) is pulled in the direction of the extreme value.

\item From Figure 3.2, we see that we have two outliers.

\item What central tendency measure is appropriate for this distribution?  Explain.

\item Are there any outliers?  If so, can we explain them?

\item Why should we use the number of social workers per 100,000 population as a measure of the availability of social worker in each state than the simple count of social workers?  \\ States vary in population.   Social Workers per 100,000 provides a better measure of how many social workers are available to serve a state.

\item Make a histogram or boxplot that displays the distribution of social workers per 100,000 population.  
