...
Seeing your result in perspective means comparing a value to distribution is important in statistics.  
...
The \emph{mean} (usually denoted as $\bar{x}$) is blank as an average of values in the dataset.

the arithmetic center of distribution and 

It is the most commonly utilized measure.  
...
Suppose that there are 13 students in STAT 2830 ages as given in Table 3.1 and that we need to obtain their mean.
...
The value of the mean is 27.9.  After looking again at the data, we see that one age value, 78, is abnormally high.  It could be a non-traditional student or it could be a data entry error.  After reviewing the numbers, we see that the value 78 is an error and that the correct value should be 18.  We change the age value 78 to 18 (Table 3.2) and recompute the mean.  
...
This example illustrates how even a single value (outlier) can affect the mean.  The mean for original dataset (with the outlier) was 27.9 and after the value 78 was changed to 18, the corrected mean is 23.3.

  
\subsubsection{Median}

We can eliminate this sensitivity to outliers by using another measure of central tendency called \emph{median}.  It is defined as the middle observation of a dataset with values arranged in order of their magnitude.  Half of the values in the dataset are below the median, and half are above the median.   After we arrange the ages (from low to high) of the students from STAT 2830 class, we find the middle value.   Using the original dataset with the outlier; see Table 3.1.  
...
\item Suppose that one student was not on the list.  She was added to the list and her age (19) was added to the dataset (see Table 3.5).  As median we will of  Finding a median is not difficult; however, the dataset used an odd number of values.  How do we find the median if the we have an even number of observations?  
...
\begin{itemize}
\item Strength:  Median is not sensitive to extreme values (\emph{outliers}).
...
Another way of locating the center of the dataset is to find the most frequent values.
