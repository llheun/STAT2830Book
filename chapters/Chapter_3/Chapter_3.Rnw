...
Seeing your result in perspective means comparing a value to distribution is important feature in statistics.  
...
\begin{itemize}
\item For numeric data types, the measure of location for such data, the \emph{mode}, is given by the value (number of objects) that most frequently occurs in a dataset, see Table 3.5.
...
\begin{itemize}
\item Sometimes we can think of the \emph{mean} as an economic measure, and we use it only for the numeric data type such as the number of staff to support the current workload at the social services office in our community.  It can never be used when we have categorical data type. 
\item At times we determine the \emph{median} as a social measure, and we use it only for the numeric data type such as median income would be of interest to welfare office in our community.  It can never be used when we have the categorical data type.
\item We do not use \emph{mode} very often, and we can use it for both categorical and numerical data types.  Occastionally, there are no repeating values of a numerical dataset which is not very useful for determining central tendency.  Other times we use the mode for central tendency of nomimal values.
\end{itemize}
}}
...
Often, we want to know the distribution of the values in our dataset.  Is the shape symmetrical (bell-shaped) or is it shewed in some direction?
...
\samepage
  ...
  \item Lower quartile is also the median of the lower half of the dataset. 
  \end{itemize}
\samepage
...
The median for 2015 is \Sexpr{med3b}.
  ...
  The admissions department at WMU gave 25 randomly selected freshmen a national prejudice survey.  Their score on the test is a xxx prejudice index that will be used these scores in three years to see if higher education affects prejudice.
...
\begin{minipage}[ht]{6cm}  
  ...
  Calculate the mean and median of the pre and post test scores.
...
The table reveals the mean, median, and mode of \emph{highest} years of school completed. 
