...
The method of seeing your result in perspective is comparing a value to distribution is important in statistics.  We will define three standard measures of central tendency.
...
\item Note: After reviewing Table 3.3 and Table 3.4, we notice that it does not change very much even after the correction.
...
What are the strengths and weaknesses of the value of the median? (See Table 3.3 and Table 3.4.)
...
\begin{itemize}
\item For numeric data types, the measure of location for such data, the \emph{mode}, is given by the value (number or object) that most frequently occurs in a dataset, see Table 3.5.

\item For categorical data types, we have some datasets that contain objects that we cannot order, e.g., the model of our car,  religious affiliation,  birth state,  eye color, gender, etc.  These are categorical data types.  We cannot use either the mean or the median for such data.  
...
We use graphic displays of numerical data to help us see models.  We commonly use these patterns in the data to describe features such as center, spread, and shape.  We often use descriptive labels, such as symmetric and skewed, to explain some common distributions.
...
\samepage
\item Percentile: a point below which a particular percentage of cases fall.  Suppose we have a dataset of 16 values (see Table 3.6) and want to know which number in the data set where 25 percent of the values fall ($25^{th}$ percentile).  First, we order the data set in ascending order.  We take .25 times 16 which equals 4.  The fourth position is the $25^{th}$ percentile, and the value is 5.23.
\samepage
\item Deciles: divides the distribution into tenth (10, 20, $\dots$, 90).  For example, often pediatrician offices use decile statistics to classify children's height in their age groups, etc.   
\samepage
\item Quartiles: divides the distribution into quarters. Referring to Table 3.6, the quartile values are  $Q_1 = 5.23$, $Q_2 = 5.27$, $Q_3 = 5.31$.
\samepage
  \begin{itemize}
...
\item Calculate a percentile
	...
	\item Example
		\begin{itemize}
		\item In a sample of 16 counties in Michigan (see Table 3.6), we want to find the $3^{rd}$ quartile (or the $75^{th}$ percentile),
		\item $16 \times 0.75 = 12$ is the $75^{th}$ percentile.  In this case, the $75^{th}$ percentile is $12^{th}$ item of the data set, i.e., the third quartile is 5.31.
		\end{itemize}
...
The Table 3.7 reports the three central tendency values: 
