...
\begin{itemize}
\item Strength -- The mean uses all data from a numeric variable.
...
After looking at the data, we see that one age value, 78, is abnormally high.  It could be a non-traditional student or it could be a data entry error.  After reviewing the numbers, we see that the value 78 should be 18.  We change the age value 78 to 18 in Table 3.2 and recompute the mean.  
...
\item Suppose that we found another student who was nineteen years old and added her to the dataset of students, we can ask how does this additional student affect the median.  In other words, if there is an even number of values in our dataset, we must order the values at the beginning and end and locate the two center values and average them (see Table 3.5), e.g., 
...
What are the strengths and weaknesses of the median? (See Table 3.3 and Table 3.4.)
...
There are two definitions for the \emph{mode}.  

\begin{itemize}
\item For numeric data types, the measure of location for such data, the \emph{mode}, is given by the most frequently occurring number in the dataset, see Table 3.5.

\item For categorical data types, we have  some datasets that contain objects that cannot be ordered, e.g., the model of our car, our religious affiliation, our birth state, our eye color, etc.  These are categorical data types.  We cannot use either the mean or the median for such data.  
...
When we use a histogram to demonstrate a bell-shaped distribution, we see a symmetric distribution that we can divide the curve at the center so that each half is a mirror image of the other, see Figure 3.1(a).

\subsubsection{Skewed}

When we  use a histogram to show a skewed distribution, we see some distributions have many more data values on one side of the histogram than the other.  When there are fewer observations on the right, i.e., toward the higher values, we say that the distribution is \emph{right skewed} (see Figure 3.1(b)).  If on the other hand, we see fewer observations on the left, i.e., toward the lower values, we say that the distribution is \emph{left skewed} (see Figure 3.1(c)).
...
\samepage
\item Quartiles: divides distribution into quarters. Referring to Table 3.6, the quartile values are  $Q_1 = 5.23$, $Q_2 = 5.27$, $Q_3 = 5.31$.
\samepage
  \begin{itemize}
...
\item Calculate a percentile
	...
	\item Example
		\begin{itemize}
		\item In a sample of 16 counties of Michigan (see Table 3.6), we want to find the $3^{rd}$ quartile (or the $75^{th}$ percentile),
		\item $16 \times 0.75 = 12$ is the $75^{th}$ percentile.  In this case, the $75^{th}$ percentile is $12^{th}$ item of the dataset, i.e., the third quartile is 5.31.
		\end{itemize}
...
The table reports the three central tendency values in the following table (Table 3.7): 
