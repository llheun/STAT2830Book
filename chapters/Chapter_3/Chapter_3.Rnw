...
\begin{enumerate}
\item Why should we use the number of social workers per 100,000 population as a measure of the availability 
of social worker in each state than the simple count of social workers?  \\ States vary in population.  
Social Workers per 100,000 provides a better measure of how many social workers are available to serve a state.
