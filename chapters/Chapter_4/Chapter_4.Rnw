...
\end{table}
	...
	\begin{solution}
        ...
        max(tbl4_1$PercentGDP) - min(tbl4_1$PercentGDP)
      @
	 
	    The IQR is 
		  <<results="asis", echo=FALSE>>=
        ## tbl4_1 <- read.csv("data/2013TEoHpGDP.csv",header=TRUE)
       IQR(tbl4_1$PercentGDP)
      @
	 
	    The standard deviation is 
		  <<results="asis", echo=FALSE>>=
        ## tbl4_1 <- read.csv("data/2013TEoHpGDP.csv",header=TRUE)
       sd(tbl4_1$PercentGDP)
      @
      
      The coefficient of variation is 
		  <<results="asis", echo=FALSE>>=
        ## tbl4_1 <- read.csv("data/2013TEoHpGDP.csv",header=TRUE)
       sd(tbl4_1$PercentGDP) / mean(tbl4_1$PercentGDP) * 100
      @
	 
      The variance is 
		  <<results="asis", echo=FALSE>>=
        ## tbl4_1 <- read.csv("data/2013TEoHpGDP.csv",header=TRUE)
       sd(tbl4_1$PercentGDP)^2 
      @
