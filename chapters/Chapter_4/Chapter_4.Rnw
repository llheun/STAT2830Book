...
\begin{exercises}
  ...
  \begin{exercise} % 2
    ...
    Given $Q_3 = 20$ and $Q_1 = 10$, an outlier is any value that is
	...
	\begin{solution}
	  ...
	  An outlier is any value that is less than -5 or greater than 35.
  ...
  \begin{exercise} % 3
	...
	\vspace{2mm}
	\begin{solution}
	  $ SD 1, 2, 3, 4, 5 is \sqrt{ \frac{10}{4 }} = 1.581 $
	\end{solution}
...
\end{table}
  ...
  Describe the spread variables of the distribution.
...
@
  ...
  \begin{exercise}	% 5
  	...
  	Using the data from Table 4.4 are there any outliers?
  ...
  Note: numerical variables can use the mean, standard deviation, median, histograms, and line charts; while categorical variables can use frequency tables, mode, bar and pie charts.
