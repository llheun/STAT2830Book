...
After reading chapter four and doing the exercises, you will be able to
\begin{itemize}
...
\item Compute and explain the following:
	...
	\end{itemize}
\item Select the  measure of dispersion appropriate for your current problem.
\end{itemize}
...
\item It only includes two values from the dataset (minimum and maximum), has limited power for interpretation, and 
 has limited use in more advanced statistical analysis of numerical data.
\end{itemize}
...
But a larger value of the standard deviation from the urban university tells us that the ages of students at the urban  campus are toward one extreme or the other.   Naturally, we should then ask why is the data skewed when students go to an urban college (See Figure 4.5) because older students attend it.  Or the residential campus mean age is dragged down because younger students participate in the residential campus classes.  By looking the standard deviation, helps us understand the data.

Needless to say, when you are evaluating the results of your research, you may seek the advice of a statistician.  However, if you have knowledge of the standard deviation, your time with the statistician will be productive.

Some of you may be asking, how do we compute the standard deviation?  For those of you who are curious, you may continue to read the remaining proportion of the section.  For the others, you may go to the next section.  Remember, there are statistical software and calculators that will compute the standard deviation for you.
\vspace{2mm}
...
\emph{Referring  to Table 4.2, Computing SD for two Campuses, we took each value $x$ and subtracted the value from the mean and then multiply that difference by itself, i.e., determined the square of the differences.  Now we can sum (add)  the squared differences and divide that result by $n - 1$.  That's it.  There is one more step.  We are required to take the square root of the number as we did in Table 4.3.  There is one more way to compute the standard deviation; that is to divide the sum of squared deviations by $n$ instead of $n - 1$.  It is beyond the scope of this text to explain it here.  If you have, grasp the basic concept of the standard deviation, you should be
okay.}
...
\item describes the ``average'' or typical squared deviation of the scores.  The statistic should give us an idea of how far scores are from each other or the center of the distribution,
...
The four  spread values are displayed in the following table (Table 4.4): 
...
\begin{exercises}
  ...
  The weekly budget for groceries from six student are as follows: 
...
@
  ...
  \end{solution}
	...
	\begin{exercise}  % 4
	  ...
	  The next 2 exercises refer to the following scenario.  Table 4.5 shows the 2013 total expenditure on health as a percentage of GDP in 34  countries.  The health expenditure per capita is the sum of private and public health expenditures divided by GDP. Health expenditures include emergency aid, family-planning services, health functions, and nutrition activities, but exclude services of sanitation and potable water.  \cite{Health2013}
...
@     
  ...
  You are preparing  for the  sociology statistics final exam with some your classmates.   A classmate has volunteered to use SPSS to make the needed charts and tables for a data set that has several categorical and numerical variables that were assigned by the statistics professor to use as a study guide.  The classmate shows you the reports and says ``See, I've got them all -- boxplots, charts, means, medians, and standard deviations for all our variables along with graphs.  Also, I don't understand why  Professor Doe told us that we cannot use  descriptive statistics for some of the variables; I got descriptive statistics for everything!  See, the mean for height is 67, the mean for GPA is 2.8. the mean for gender is 1.45, the mean for major course of study is 4.56.''  However, as you review the report  something about the output appears weird -- like boxplots for gender and  major course of study and pie charts for grade point average and height.  What is your reply?
...
\end{table}
  ...
  Calculate the range and standard deviation pre and post test scores.
