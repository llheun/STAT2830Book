...
\item Thus, we are 95 percent confident that the true population proportion for readmission is between 43.2\% and 62.8\%.  More discussion???????
\end{enumerate}
...
\begin{exercises}
  ...
  \begin{solution}
      \begin{align*}
      SE &= \frac{.3}{\sqrt{100}} = 0.03 \\
      ME &=  t_{99,0.025} \times SE = 1.980 \times 0.03 = 0.0594 
    \end{align*}
  \end{solution}
  ...
  \begin{exercise} % 6 
    ...
    The next 5 questions refer to the following scenario. 
    Occasionally, the World Values Survey randomly selects samples from various nations from around the world.  In the table below are respondents from two nations who said that they are either ``happy or very happy.''  Calculate the sample proportions and construct a 95 percent confidence interval for each selected nation.
...
\end{table}
  ...
  \begin{exercise} % 8
    ...
    What is the critical value for all nations at the significance level $\alpha = 0.05$ for a two-tailed test?
