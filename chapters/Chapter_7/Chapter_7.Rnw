...
Figure 7.1 shows the idea of a 95 percent confidence interval (CI) in a different form.  It illustrates 50 simple random samples drawn from the same population and computing 95 percent CI from each sample.  At the center of each interval is $\bar{x}$ and consequently, varies from sample to sample.  The smapling distribution of $\bar{x}$ appears at the bottom of the graph to illustrate the long term pattern of the variation.  The population $\mu$ is at the center of the sampling distribution.  The 95 percent CI from 50 simple random samples are shown above.  The center $\bar{x}$ of each interval is marked with a small circle.  The lines on either side of the circle span the CI.  Most of the 50 CI capture the true value of the population mean $(\mu)$.  The theory says that if we take thousands of smaples, 95 percent of the CI will contain $\mu$.


<<f7_1, echo=FALSE, fig.pos="ht", fig.align="center", fig.width=4, fig.height=4, fig.cap="Box plot of Years of Education">>=
