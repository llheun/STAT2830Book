...
In the preceding chapters we discussed the differences between a population and a sample.  The sample which is a portion of a population of interest, such as researchers found that 15 of 200 households consisted of unmarried couples who were living together.  The reason  for collecting this data the sample statistic are     ???????????????
