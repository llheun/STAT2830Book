...
\section{Objective}
After reading chapter seven and doing the exercises, you will be able to
\begin{itemize}
...
A random sample was selected of 100 patients treated in a program for alcoholism and drug dependency over the past 10 years.  It was determined that 53 of the patients had been readmitted to the program at least once.  Find a 95 percent confidence interval for the population proportion for readmission.  (Some people might start drinking again but not come for a treatment for the second time.)
...
\item Thus, we are 95 percent confident that the true population proportion for readmission to a program for alcoholism and drug dependency is between 43.2\% and 62.8\%.  Suppose you want a 99 percent confidence level.  You find $\alpha = 0.01$ and the CV = 2.5758 amd the ME = 0.129.  

The higher CI of 99 percent yields an interval of the true population proportion for readmission to a program for alcoholism and drug dependency is between 40.1\% and 65.9\%.  Given the current sample size of 100 persons; 53  were readmitted to the program for 
alcoholism and drug dependency.  You gain confidence (from 95\% to 99\%) at the expense of precision, as the ME increases from 0.098 to 0.129.
\end{enumerate}
