...
\item The one-sample case requires that a single random sample is selected while the two-sample case requires the two independent random samples are selected.
	\begin{itemize}
	\item This means that selecting samples from one of the sample will not affect the probabilities of selecting samples for the other case.
	\end{itemize}
...
Did the program work?  (Or at five percent level of significance, is the proportion of divorces of clients on particular program significantly lower than the proportion of divorces of clients on the regular program?)
