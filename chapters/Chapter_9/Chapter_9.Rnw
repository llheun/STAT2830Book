...
% Requires the booktabs if the memoir class is not being used
  ...
  \begin{exercise} % 6
    ...
    The next four quesions refer to the following scenario.
    Suppose about half of the public safety officers in Kalamazoo, Michigan, had finalize a special curriculum in investigative procedures.  Did the course increase their effectiveness in clearing crimes by arrest?  The results of the survey are reported below:
  ...
  \begin{exercise} % 8 
      ...
      What is the sampling distribution and critical value?  Use a 5 percent significance to test if the difference in significant?
	  \begin{enumerate}
	  \samepage
	  \item Standard Normal (z) distribution, $Z = \pm1.96$.
	  \samepage
  ...
  \vspace{2mm}
  \begin{solution}
    Standard Normal (z) distribution, $Z = \pm1.645$.
  \end{solution}
