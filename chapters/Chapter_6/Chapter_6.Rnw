...
\item Following descriptive statistics will be used to make inference about the population: 
	...
	\end{itemize}
\samepage
\item When we selecting a sample, it is critical that the sample is {\bf{representative}} of the population.  Otherwise, the calculated statistics may have a {\bf{bias}} and our conclusions will not be valid.
\samepage
...
\item {\bf{Simple Random Sampling}} -- Samples are randomly selected from the population.  It can be done using the lottery method, table of random numbers, computer or calculator random number generator. 
	\begin{itemize} 
	\item Example: {\bf{Target Population}} -- Households in Kalamazoo; \\ {\bf{Simple Random Sampling}} -- randomly selecting  samples from a list of all families in Kalamazoo.
	\end{itemize}
...
\end{minipage}
 ...
 \item But using the average height of men in the United States, we find that 99.84 percent of the groups means will be from 66.2 and 72.0 inches.
 \end{itemize}
