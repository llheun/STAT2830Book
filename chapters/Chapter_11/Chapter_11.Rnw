...
\chapter{Testing Equality of two or more Averages }
\label{chap:ch11}
...
%  \caption{Descriptive Statistics: Percent voter turnout by Election Type}
   ...
   \begin{tabular}{@{} lccc @{}} \hline % Column formatting, @{} suppresses leading/trailing space
      & \multicolumn{3}{c}{Elect.Type } \\ \hline 
      % \cmidrule(r){1-2} % Partial rule. (r) trims the line a little bit on the right; (l) & (lr) also possible
...
\item Step 2.  Stating the Hypotheses.
 ...
 $H_a:$ at least on of the population means $(\mu)$ is different. \\
...
<<f11_1, echo=FALSE, fig.pos="ht", fig.align="center", fig.width=4, fig.height=4, fig.cap="Voter Turnout by Election Type">>=
...
\item Looking at the confidence interval plot, it can be seen that the confidence interval for the ``Local'' does not overlap the confidence interval for ``National.'' 

\item We can then conclude that the Local is less than National, but cannot conclude that either is different from State voter rates.
