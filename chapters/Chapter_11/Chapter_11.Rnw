...
\begin{exercises}
  ...
  \end{solution} 
    ...
    STEP 2: Using the information from exercise three, we must state the hypotheses about its relationship with having a gun in the home.  Based on our experience, how should we state our hypotheses?
  ...
  \end{solution} 
    ...
    STEP 3: Getting the output from GSS2014 dataset, we will use SPSS Compare Means. We will start the procedure by selecting the drop down menu Analysis, then select One-way ANOVA. The One-way ANOVA dialog box shows the list of variables on the left. Highlight the name of our dependent variable (Age of respondent [age]]) and click the arrow to move {\it{age}} into the Dependent List box. Next we need to find the Factor variable ({\it{confidence in the banking and financial systems}} [Confinan]) and move it into the Factor box. Click the Options button and select Descriptive and click on Continue and OK to produce our output.
